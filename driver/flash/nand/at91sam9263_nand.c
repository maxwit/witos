...
static void at91_nand_cmd(struct nand_chip *nand, int nCmd, unsigned int nCtrl)
{
	...
	if (nCtrl & NAND_CLE)
		writeb((char *)nfc->data_reg + NAND_CMMD, nCmd);
	else
		writeb((char *)nfc->data_reg + NAND_ADDR, nCmd);
}
