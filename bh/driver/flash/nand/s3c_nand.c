...
#include <arm/s3c_nand.h>

static struct nand_oob_layout g_s3c_oob16_layout =
{
	.ecc_code_len = 3,
	.ecc_pos    = {0, 1, 2},
	.free_region = {{8, 8}}
...
static struct nand_oob_layout g_s3c_oob64_layout =
{
	.ecc_code_len = 24,
	.ecc_pos =
	{
...
static int s3c_nand_is_ready(struct nand_chip *nand);
static void  s3c_nand_read_buff(struct nand_ctrl *nfc, u8 *buff, int size);
static void  s3c_nand_write_buff(struct nand_ctrl *nfc, const u8 *buff, int size);
static void s3c_nand_enable_hwecc(struct nand_chip *nand, int mode);
static int s3c_nand_calc_hwecc(struct nand_chip *nand, const u8 *data, u8 *ecc);
static int s3c_nand_correct_data(struct nand_chip *nand,
								u8 *data, u8 *ecc_read, u8 *ecc_calc);

static int s3c_nand_init(struct nand_ctrl *nfc);
static int s3c_nand_init(struct nand_ctrl *nfc);

static int __INIT__ s3c_nand_probe(void);
...
static int s3c_nand_calc_hwecc(struct nand_chip *nand, const u8 *data, u8 *ecc)
{
	...
	val = readl(VA(NAND_CTRL_BASE + NFMECC0));
#endif
...
static int __INIT__ s3c_nand_probe(void)
{
	...
	nfc->flash_ready  = s3c_nand_is_ready;
	nfc->read_buff    = s3c_nand_read_buff;
	nfc->write_buff   = s3c_nand_write_buff;
	// ECC
	nfc->ecc_enable   = s3c_nand_enable_hwecc;
	nfc->ecc_generate = s3c_nand_calc_hwecc;
	nfc->ecc_correct  = s3c_nand_correct_data;
	...
	for (i = 0; i < nfc->max_slaves; i++) //
	{
		...
		{
			nfc->hard_oob_layout = &g_s3c_oob16_layout;
			nfc->ecc_data_len    = 512;
		}
		else
		{
			nfc->hard_oob_layout = &g_s3c_oob64_layout;
