...
int s3c6410_lcdc_init(void)
{
	...
	//init register
	lcd_6410_writel(VIDCON0, 0 << 29 | 0 << 26 | 0 << 17 | 0 << 16 | clk_val << 6 | 1 << 4 |0 << 2 | 0x3);
	lcd_6410_writel(VIDCON1, 0 << 5);
	...
	lcd_6410_writel(WINCON0, 1 << 16 | 0x05 << 2 | 1);
	lcd_6410_writel(VIDOSD0A, 0);
	lcd_6410_writel(VIDOSD0B, vm->width << 11 | vm->height);
	lcd_6410_writel(VIDOSD0C, vm->width*vm->height);

	//vidosd0a b c0
	lcd_6410_writel(VIDW00ADD0B0, dma);
	lcd_6410_writel(VIDW00ADD1B0, (dma & 0xffffff) + (vm->width + 512)*(vm->height + 1));
	lcd_6410_writel(VIDW00ADD2, vm->width);
	printf("dma= %08x, %p\n", dma, va);
